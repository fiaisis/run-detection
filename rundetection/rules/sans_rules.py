...
def _get_scatter_title(scatter: SansFileData) -> str:
    if "}_{" in scatter.title:
        return scatter.title.split("}_{")[0].lstrip("{").rstrip("}")
    return scatter.title.split("}_")[0].lstrip("{").rstrip("}")
...
def _grab_file_prefix(job_file_path: Path) -> str:
    return re.sub(r"\d+", "", job_file_path.stem)
...
def _find_direct(job_request: JobRequest) -> SansFileData | None:
    for empty_shorthand in ("direct", "empty", "mt", " mt", "mt ", "{mt}"):
        direct_files = _find_all_files_in_journal_on_condition(
        ...
        for file in direct_files:
            if _is_direct_file(job_request=job_request, file_path=_generate_direct_file_path(job_request.filepath,
...
def _find_all_files_in_journal_on_condition(condition: Callable) -> iter:
    return filter(condition, SANS_FILES)


def _find_file_in_journal_by_title_and_type(title: str, file_types: set[str]) -> SansFileData | None:
    found_files = list(_find_all_files_in_journal_on_condition(lambda file: file.title.strip("{").strip("}").lower() == title.lower() and file.type in file_types))
...
class SansCanFiles(Rule[bool]):
    ...
    def verify(self, job_request: JobRequest) -> None:
        ...
        if background_title:
            ...
            # Set it as the direct file
            background_scatter = SansFileData(title=job_request.additional_values["scatter_direct_title"], type="SANS",
                                              run_number=job_request.additional_values["scatter_direct"])
        background_trans = _find_trans(scatter=background_scatter, job_request=job_request)
        if background_trans is None:
            logger.error("Background file %s does not have a findable Transmission file", background_title)
        else:
